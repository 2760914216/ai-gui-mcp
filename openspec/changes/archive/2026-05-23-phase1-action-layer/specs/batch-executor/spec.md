...
- **THEN** `action1` and `action2` execute successfully, the batch returns `{"completed": 2, "total": 3, "error": "<error detail>"}`

#### Scenario: All actions succeed
- **WHEN** `batch(actions=[action1, action2, action3])` is called and all succeed
- **THEN** the batch returns `{"completed": 3, "total": 3}`
...
The batch executor SHALL support `tool: "screen"` with `action: "size"` in the actions list, returning screen size results inline.
