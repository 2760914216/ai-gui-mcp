...
**Reason**: P2 spike phase is complete. The prototype in `spike/` directory has validated the D-Bus approach and its results are documented in `docs/PHASE2-SPIKE-RESULTS.md`. Production code replaces the prototype.

**Migration**: Remove `spike/` directory scripts. Production `XdgPortalBackend` is implemented in `src/backends/portal.py`.
