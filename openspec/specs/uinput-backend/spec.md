...
The system SHALL implement `get_cursor_position()` returning the internally tracked cursor coordinates `(self._x, self._y)`.

#### Scenario: Cursor position after initialization
- **WHEN** `get_cursor_position()` is called immediately after `UInputBackend()` initialization
- **THEN** the return value is `(0, 0)`

#### Scenario: Cursor position after move_abs
- **WHEN** `move_abs(500, 300)` has been called and `get_cursor_position()` is invoked
- **THEN** the return value is `(500, 300)`

### Requirement: UInputBackend warns on startup when cursor position is unknown
The system SHALL log a warning message to stderr at initialization time indicating that the cursor position is unknown and tracking starts at (0,0).

#### Scenario: Startup warning logged
- **WHEN** `UInputBackend()` is initialized
- **THEN** the message `[ai-gui-mcp] cursor position unknown, tracking assumes (0,0)` is written to stderr
